...
- **Map config guardrails.** The sanitization helper now has explicit coverage for empty inputs, traversal/device specifiers, path separators, and other illegal characters via `tests/test_map_config_filename_sanitization.cpp`; exercise it locally with `python3 tools/ci/run_tests.py` before shipping related changes.【F:tests/test_map_config_filename_sanitization.cpp†L1-L81】【F:tools/ci/run_tests.py†L1-L112】
...
- Exhaustive mapping of spawnable entities, items, and spawnflags: [Level Design Entity Catalogue](level-design.md#entity-catalogue).【F:docs/wiki/level-design.md†L15-L248】
