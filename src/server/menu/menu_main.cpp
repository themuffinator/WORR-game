...
void Menu::Render(gentity_t* ent) const {
	...
	const bool hasAbove = (std::ranges::distance(entries
		| std::views::filter([](const MenuEntry& entry) {
			return entry.scrollable;
		})
		| std::views::take(offset)) > 0);
	const bool hasBelow = (std::ranges::distance(entries
		| std::views::filter([](const MenuEntry& entry) {
			return entry.scrollable;
		})
		| std::views::drop(offset + MAX_VISIBLE_LINES)) > 0);
