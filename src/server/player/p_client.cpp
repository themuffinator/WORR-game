...
Loads an existing player config or writes a default when missing.
=============
*/
static void PCfg_ClientInitPConfig(gentity_t* ent) {
	...
	if (!file_exists) {
		...
		const std::filesystem::path config_dir = "baseq2/pcfg";
		std::filesystem::create_directories(config_dir, create_error);
		if (create_error) {
			gi.Com_PrintFmt("{}: Failed to ensure player config directory \"{}\": {}\n", __FUNCTION__, config_dir.string(), create_error.message());
			return;
		}
