...
void Drop_Weapon(gentity_t *ent, Item *inv);
...
static void HighValuePickupCounter(gentity_t *ent, gentity_t *other) {
	other->client->pers.match.pickupCounts[ent->item->highValue]++;
	other->client->pers.match.pickupDelay[ent->item->highValue] += level.time - ent->timeStamp;
...
static bool Tech_PlayerHasATech(gentity_t *ent) {
	if (Tech_Held(ent) != nullptr) {
		if (level.time - ent->client->techLastMessageTime > 10_sec) {
			gi.LocCenter_Print(ent, "$g_already_have_tech");
			ent->client->techLastMessageTime = level.time;
		}
...
static bool Tech_Pickup(gentity_t *ent, gentity_t *other) {
	...
	other->client->techRegenTime = level.time;
	return true;
}
...
int Tech_ApplyDisruptorShield(gentity_t *ent, int dmg) {
	...
	if (ent->client && ent->client->powerupTime.silencerShots)
		volume = 0.2f;
...
bool Tech_ApplyPowerAmpSound(gentity_t *ent) {
	...
	if (ent->client &&
		...
		if (ent->client->techSoundTime < level.time) {
			ent->client->techSoundTime = level.time + 1_sec;
...
void Tech_ApplyTimeAccelSound(gentity_t *ent) {
	...
	if (ent->client &&
		ent->client->pers.inventory[IT_TECH_TIME_ACCEL] &&
		ent->client->techSoundTime < level.time) {
		ent->client->techSoundTime = level.time + 1_sec;
...
void Tech_ApplyAutoDoc(gentity_t *ent) {
	...
	bool		no_health = mod || GTF(GTF_ARENA) || !game.spawnHealth;
	...
	if (cl->powerupTime.silencerShots)
		volume = 0.2f;

	if (mod && !cl->techRegenTime) {
		cl->techRegenTime = level.time;
		return;
	}
	...
	if (cl->techRegenTime < level.time) {
		gtime_t delay = 1_sec;
		...
		if (!no_health && !noise) {
			...
			if (index && cl->pers.inventory[index] < max) {
				...
				cl->techRegenTime += 1_sec;
				noise = true;
			}
		}
	}
	if (noise && cl->techSoundTime < level.time) {
...
static inline item_id_t FindSubstituteItem(gentity_t *ent) {
	...
	for (item_id_t i = static_cast<item_id_t>(IT_NULL + 1); i < IT_TOTAL; i = static_cast<item_id_t>(static_cast<int32_t>(i) + 1)) {
		...
		if (!add) {
			...
			if (!game.spawnPowerups && itflags & (IF_POWERUP | IF_SPHERE))
				continue;

			if (!game.spawnBFG && ent->item->id == IT_WEAPON_BFG)
				continue;
...
THINK(RespawnItem)(gentity_t *ent) -> void {
	...
	if (ent->team) {
		...
		if (GT(GT_CTF) && match_weaponsStay->integer && master->item && (master->item->flags & IF_WEAPON)) {
			ent = master;
		} else {
			// Hide current item
...
static void Drop_General(gentity_t *ent, Item *item) {
	...
	if (item->flags & IF_POWERUP) {
		...
		case IT_POWERUP_SILENCER:
			ent->client->powerupTime.silencerShots = 0;
			break;
...
static inline bool G_AddIDAmmoAndCapQuantity(gentity_t *other, item_id_t ammoID) {
	Item *item = GetItemByAmmo((ammo_t)itemList[ammoID].tag);
...
static bool Pickup_Pack(gentity_t *ent, gentity_t *other) {
	...
	Item *it = GetItemByIndex(IT_AMMO_GRENADES);
	if (it)
...
static bool Pickup_PowerArmor(gentity_t *ent, gentity_t *other) {
	...
	G_AddAmmoAndCapQuantity(other, AMMO_CELLS, 20);
...
bool Entity_IsVisibleToPlayer(gentity_t *ent, gentity_t *player) {
	...
	return !ent->item_picked_up_by[index];
}
...
TOUCH(Touch_Item) (gentity_t *ent, gentity_t *other, const trace_t &tr, bool otherTouchingSelf) -> void {
	...
	if (coop->integer && P_UseCoopInstancedItems()) {
		const int32_t index = other->s.number - 1;
		if (index < 0 || index >= MAX_CLIENTS)
			return;

		if (ent->item_picked_up_by[index])
			return;
	}
...
bool CheckItemEnabled(Item *item) {
	...
	if (!add) {
		if (!game.spawnArmor && item->flags & IF_ARMOR)
			return false;

		if (!game.spawnPowerArmor && item->flags & IF_POWER_ARMOR)
			return false;

		if (!game.spawnPowerups && item->flags & IF_POWERUP || ((CooperativeModeOn() || !deathmatch->integer) && skill->integer > 3))
			return false;

		if (!game.spawnBFG && item->id == IT_WEAPON_BFG)
			return false;
...
bool SpawnItem(gentity_t *ent, Item *item) {
	...
	if (item->flags & IF_WEAPON && item->id >= FIRST_WEAPON && item->id <= LAST_WEAPON)
		level.weaponCount[item->id - FIRST_WEAPON]++;
	...
	*/
		{
			...
			/* pickup_name_definite */ "Mega Health",
			/* quantity */ 100,
			/* ammo */ IT_NULL,
			/* chain */ IT_NULL,
			/* flags */ IF_HEALTH,
	...
	*/
		{
			...
			/* view_model */ nullptr,
			/* icon */ "i_ctfn",
			/* use_name */  "Neutral Flag",
			/* pickup_name */  "Neutral Flag",
			/* pickup_name_definite */ "Neutral Flag",
			/* quantity */ 0,
			/* ammo */ IT_NULL,
			/* chain */ IT_NULL,
			/* flags */ IF_NONE,
...
void InitItems() {
	...
	for (item_id_t i = IT_NULL; i < IT_TOTAL; i = static_cast<item_id_t>(i + 1)) {
		...
		Item *chain_item = &itemList[item->chain];

		if (!chain_item)
			gi.Com_ErrorFmt("Invalid item chain {} for {}", (int32_t)item->chain, item->pickup_name);

		// set up initial chain
		if (!chain_item->chainNext)
			chain_item->chainNext = chain_item;

		// if we're not the first in chain, add us now
		if (chain_item != item) {
			Item *c;

			// end of chain is one whose chainNext points to chain_item
			for (c = chain_item; c->chainNext != chain_item; c = c->chainNext)
				continue;

			// splice us in
			item->chainNext = chain_item;
			c->chainNext = item;
		}
	}
