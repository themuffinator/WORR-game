...
This script updates the VERSION file and regenerates src/shared/version.hpp.
"""
...
HEADER_FILE = REPO_ROOT / "src" / "shared" / "version.hpp"
...
inline constexpr int kMajor = {major};
inline constexpr int kMinor = {minor};
inline constexpr int kPatch = {patch};
inline constexpr bool kHasPrerelease = {has_prerelease};
inline constexpr bool kHasBuildMetadata = {has_build};
inline constexpr const char* kPrerelease = "{prerelease}";
inline constexpr const char* kBuildMetadata = "{build}";
...
def write_header(version: SemVer) -> None:
    header_contents = HEADER_TEMPLATE.format(
        ...
        minor=version.minor,
        patch=version.patch,
        has_prerelease="true" if version.prerelease else "false",
        has_build="true" if version.build else "false",
...
def main(argv: list[str]) -> int:
    ...
    if staged:
        print("Files staged: VERSION, src/shared/version.hpp")
    else:
        print("Run 'git add VERSION src/shared/version.hpp' to stage the changes.")
