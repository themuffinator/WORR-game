...
#include "../g_local.hpp"
#include "../client/client_session_service.hpp"
...
===============
*/
static bool PCfg_BuildConfigPath(const char* caller, const gentity_t* ent, std::string& path, std::string& sanitizedSocialID, const char* operation) {
	if (!ent || !ent->client)
		return false;
...
void PCfg_WriteConfig(gentity_t* ent) {
	...
	std::string path;
	if (!PCfg_BuildConfigPath(__FUNCTION__, ent, path, sanitizedSocialID, "write"))
		return;
...
void PCfg_ClientInitPConfig(gentity_t* ent) {
	...
	std::string sanitizedSocialID;
	std::string path;
	if (!PCfg_BuildConfigPath(__FUNCTION__, ent, path, sanitizedSocialID, "read")) {
		return;
	}
