...
static constexpr int   SCRAG_DAMAGE = 10;
static constexpr int   SCRAG_SPEED = 500;
static const Vector3    SCRAG_MUZZLE_OFFSET = { 0.0f, 0.0f, 30.0f };
...
static void scrag_idle(gentity_t* self) {
        if (frandom() < 0.15f) {
                const auto& sound = (frandom() < 0.5f) ? s_idle1 : s_idle2;
                gi.sound(self, CHAN_VOICE, sound, 1, ATTN_IDLE, 0);
        }
}
...
static void ScragTeleport(gentity_t* self) {
        if (skill->integer <= 3 && frandom() >= 0.5f)
                return;

        TryRandomTeleportPosition(self, 128.0f, 1_sec);
}
...
static void scrag_precache() {
        ...
        s_idle1.assign("wizard/widle1.wav");
        s_idle2.assign("wizard/widle2.wav");
        s_sight.assign("wizard/wsight.wav");
        s_pain.assign("wizard/wpain.wav");
        s_death.assign("wizard/wdeath.wav");
        s_attack.assign("wizard/wattack.wav");
}
