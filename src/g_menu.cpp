...
bool Vote_Menu_Active(Entity *ent) {
...
static void G_Menu_SetHostName(PMenu *p) {
	Q_strlcpy(p->text, hostname->string, sizeof(p->text));
}

static void G_Menu_SetGamemodName(PMenu *p) {
	Q_strlcpy(p->text, level.gamemod_name, sizeof(p->text));
}

static void G_Menu_SetGametypeName(PMenu *p) {
	Q_strlcpy(p->text, level.gametype_name, sizeof(p->text));
}

static void G_Menu_SetLevelName(PMenu *p) {
...
void G_Menu_ReturnToMain(Entity *ent, MenuHandle *p);
...
void G_Menu_Admin_UpdateSettings(Entity *ent, MenuHandle *setmenu);
void G_Menu_Admin(Entity *ent, MenuHandle *p);

static void G_Menu_Admin_SettingsApply(Entity *ent, MenuHandle *p) {
...
static void G_Menu_Admin_SettingsCancel(Entity *ent, MenuHandle *p) {
	P_Menu_Close(ent);
	G_Menu_Admin(ent, p);
}

static void G_Menu_Admin_ChangeMatchLen(Entity *ent, MenuHandle *p) {
	admin_settings_t *settings = (admin_settings_t *)p->arg;
...
static void G_Menu_Admin_ChangeMatchSetupLen(Entity *ent, MenuHandle *p) {
	admin_settings_t *settings = (admin_settings_t *)p->arg;

	G_Menu_Admin_UpdateSettings(ent, p);
}

static void G_Menu_Admin_ChangeMatchStartLen(Entity *ent, MenuHandle *p) {
	admin_settings_t *settings = (admin_settings_t *)p->arg;

	G_Menu_Admin_UpdateSettings(ent, p);
}

static void G_Menu_Admin_ChangeWeapStay(Entity *ent, MenuHandle *p) {
	admin_settings_t *settings = (admin_settings_t *)p->arg;
...
static void G_Menu_Admin_ChangeInstantItems(Entity *ent, MenuHandle *p) {
	admin_settings_t *settings = (admin_settings_t *)p->arg;
...
static void G_Menu_Admin_ChangePowerupDrop(Entity *ent, MenuHandle *p) {
	admin_settings_t *settings = (admin_settings_t *)p->arg;
...
static void G_Menu_Admin_ChangeInstantWeap(Entity *ent, MenuHandle *p) {
	admin_settings_t *settings = (admin_settings_t *)p->arg;
...
static void G_Menu_Admin_ChangeMatchLock(Entity *ent, MenuHandle *p) {
	admin_settings_t *settings = (admin_settings_t *)p->arg;
...
static void G_Menu_Admin_Settings(Entity *ent, MenuHandle *p) {
	...
	menu = P_Menu_Open(ent, def_setmenu, -1, sizeof(def_setmenu) / sizeof(PMenu), settings, nullptr);
	G_Menu_Admin_UpdateSettings(ent, menu);
}

static void G_Menu_Admin_MatchSet(Entity *ent, MenuHandle *p) {
	P_Menu_Close(ent);

	if (level.matchState <= MatchState::MATCH_COUNTDOWN) {
		gi.Broadcast_Print(PRINT_CHAT, "Match has been forced to start.\n");
		Match_Start();
	} else if (level.matchState == MatchState::MATCH_IN_PROGRESS) {
		gi.Broadcast_Print(PRINT_CHAT, "Match has been forced to terminate.\n");
		Match_Reset();
	}
}

static void G_Menu_Admin_Cancel(Entity *ent, MenuHandle *p) {
	P_Menu_Close(ent);
}
...
void G_Menu_Admin(Entity *ent, MenuHandle *p) {
	...
	P_Menu_Close(ent);
	P_Menu_Open(ent, adminmenu, -1, sizeof(adminmenu) / sizeof(PMenu), nullptr, nullptr);
}

/*-----------------------------------------------------------------------*/

const PMenu pmstatsmenu[] = {
...
static void G_Menu_PMStats_Update(Entity *ent) {

	if (!g_matchstats->integer) return;

	PMenu *entries = ent->client->menu->entries;
	client_match_stats_t *st = &ent->client->pers.match;
	int i = 0;
...
static void G_Menu_PMStats(Entity *ent, MenuHandle *p) {
	P_Menu_Close(ent);
	P_Menu_Open(ent, pmstatsmenu, -1, sizeof(pmstatsmenu) / sizeof(PMenu), nullptr, G_Menu_PMStats_Update);
}
...
void G_Menu_CallVote_Map(Entity *ent, MenuHandle *p);
void G_Menu_CallVote_NextMap(Entity *ent, MenuHandle *p);
void G_Menu_CallVote_Restart(Entity *ent, MenuHandle *p);
void G_Menu_CallVote_GameType(Entity *ent, MenuHandle *p);
void G_Menu_CallVote_TimeLimit_Update(Entity *ent);
void G_Menu_CallVote_TimeLimit(Entity *ent, MenuHandle *p);
void G_Menu_CallVote_ScoreLimit(Entity *ent, MenuHandle *p);
void G_Menu_CallVote_ShuffleTeams(Entity *ent, MenuHandle *p);
void G_Menu_CallVote_BalanceTeams(Entity *ent, MenuHandle *p);
void G_Menu_CallVote_Unlagged(Entity *ent, MenuHandle *p);
void G_Menu_CallVote_Cointoss(Entity *ent, MenuHandle *p);
void G_Menu_CallVote_Random(Entity *ent, MenuHandle *p);

void G_Menu_CallVote_Map_Selection(Entity *ent, MenuHandle *p);

const PMenu pmcallvotemenu[] = {
...
const PMenu pmcallvotemenu_map[] = {
...
const PMenu pmcallvotemenu_timelimit[] = {
...
void G_Menu_CallVote_Map_Selection(Entity *ent, MenuHandle *p) {
	...
	VoteCommand *cc = FindVoteCmdByName("map");
...
static void G_Menu_CallVote_Map_Update(Entity *ent) {
...
void G_Menu_CallVote_Map(Entity *ent, MenuHandle *p) {
	P_Menu_Close(ent);
	P_Menu_Open(ent, pmcallvotemenu_map, -1, sizeof(pmcallvotemenu_map) / sizeof(PMenu), nullptr, G_Menu_CallVote_Map_Update);
}
...
void G_Menu_CallVote_TimeLimit(Entity *ent, MenuHandle *p) {
	...
	//VoteCommandStore(ent);
	P_Menu_Close(ent);
	P_Menu_Open(ent, pmcallvotemenu_timelimit, -1, sizeof(pmcallvotemenu_timelimit) / sizeof(PMenu), nullptr, G_Menu_CallVote_TimeLimit_Update);
}
...
static void G_Menu_CallVote_Update(Entity *ent) {
	PMenu *entries = ent->client->menu->entries;
	int i = 0;
...
static void G_Menu_CallVote(Entity *ent, MenuHandle *p) {
	P_Menu_Close(ent);
	P_Menu_Open(ent, pmcallvotemenu, -1, sizeof(pmcallvotemenu) / sizeof(PMenu), nullptr, G_Menu_CallVote_Update);
}
/*-----------------------------------------------------------------------*/

static void G_Menu_Vote_Yes(Entity *ent, MenuHandle *p) {
	level.vote_yes++;
...
static void G_Menu_Vote_No(Entity *ent, MenuHandle *p) {
	level.vote_no++;
	ent->client->pers.voted = -1;
...
static void G_Menu_Vote_Update(Entity *ent) {
...
void G_Menu_Vote_Open(Entity *ent) {
	P_Menu_Open(ent, votemenu, -1, sizeof(votemenu) / sizeof(PMenu), nullptr, G_Menu_Vote_Update);
}


/*-----------------------------------------------------------------------*/

static void G_Menu_Join_Team_Free(Entity *ent, MenuHandle *p) {
	SetTeam(ent, team_t:: TEAM_FREE, false, false, false);
}

static void G_Menu_Join_Team_Red(Entity *ent, MenuHandle *p) {
	SetTeam(ent, !g_teamplay_allow_team_pick->integer ? PickTeam(-1) : team_t:: TEAM_RED, false, false, false);
}

static void G_Menu_Join_Team_Blue(Entity *ent, MenuHandle *p) {
...
static void G_Menu_Join_Team_Spec(Entity *ent, MenuHandle *p) {
	SetTeam(ent, team_t:: TEAM_SPECTATOR, false, false, false);
}

void G_Menu_ReturnToMain(Entity *ent, MenuHandle *p);
void G_Menu_ChaseCam(Entity *ent, MenuHandle *p);
void G_Menu_HostInfo(Entity *ent, MenuHandle *p);
void G_Menu_ServerInfo(Entity *ent, MenuHandle *p);
...
static void G_Menu_NoChaseCamUpdate(Entity *ent) {
	PMenu *entries = ent->client->menu->entries;
...
void G_Menu_ChaseCam(Entity *ent, MenuHandle *p) {
	...
	P_Menu_Close(ent);
	P_Menu_Open(ent, nochasemenu, -1, sizeof(nochasemenu) / sizeof(PMenu), nullptr, G_Menu_NoChaseCamUpdate);
}
...
static void G_Menu_HostInfo_Update(Entity *ent) {
	PMenu *entries = ent->client->menu->entries;
	int		i = 0;
	bool	limits = false;
...
void G_Menu_HostInfo(Entity *ent, MenuHandle *p) {
	P_Menu_Close(ent);
	P_Menu_Open(ent, hostinfomenu, -1, sizeof(hostinfomenu) / sizeof(PMenu), nullptr, G_Menu_HostInfo_Update);
}

static void G_Menu_ServerInfo_Update(Entity *ent) {
	PMenu *entries = ent->client->menu->entries;
	int		i = 0;
	bool	limits = false;
...
void G_Menu_ServerInfo(Entity *ent, MenuHandle *p) {
	P_Menu_Close(ent);
	P_Menu_Open(ent, svinfomenu, -1, sizeof(svinfomenu) / sizeof(PMenu), nullptr, G_Menu_ServerInfo_Update);
}

static void G_Menu_GameRules_Update(Entity *ent) {
	PMenu *entries = ent->client->menu->entries;
	int		i = 0;
	bool	limits = false;
...
static void G_Menu_GameRules(Entity *ent, MenuHandle *p) {
	P_Menu_Close(ent);
	P_Menu_Open(ent, svinfomenu, -1, sizeof(svinfomenu) / sizeof(PMenu), nullptr, G_Menu_GameRules_Update);
}

static void G_Menu_Join_Update(Entity *ent) {
	PMenu *entries = ent->client->menu->entries;
	...
	if (Teams()) {
		if (!g_teamplay_allow_team_pick->integer && !level.locked[TEAM_RED] && !level.locked[TEAM_BLUE]) {
			...
			if (level.locked[TEAM_RED] || level.matchState >= MatchState::MATCH_COUNTDOWN && g_match_lock->integer) {
			...
			if (level.locked[TEAM_BLUE] || level.matchState >= MatchState::MATCH_COUNTDOWN && g_match_lock->integer) {
		...
		if (level.locked[TEAM_FREE] || level.matchState >= MatchState::MATCH_COUNTDOWN && g_match_lock->integer) {
...
void G_Menu_Join_Open(Entity *ent) {
	if (Vote_Menu_Active(ent))
		return;
	...
	if (Teams()) {
		...
		P_Menu_Open(ent, teams_join_menu, team, sizeof(teams_join_menu) / sizeof(PMenu), nullptr, G_Menu_Join_Update);
	} else {
		P_Menu_Open(ent, free_join_menu, team_t:: TEAM_FREE, sizeof(free_join_menu) / sizeof(PMenu), nullptr, G_Menu_Join_Update);
	}
}

// =======================================

static void G_Menu_MapSelector_SelectArenaOne(Entity *ent, MenuHandle *p) {
	int clientNum = ent - g_entities;
	if (clientNum >= 0 && clientNum < MAX_CLIENTS)
		level.mapSelectorVoteByClient[clientNum] = 0;

	P_Menu_Close(ent);
	gi.local_sound(ent, CHAN_AUTO, gi.soundindex("misc/menu3.wav"), 1, ATTN_NONE, 0);
}

static void G_Menu_MapSelector_SelectArenaTwo(Entity *ent, MenuHandle *p) {
	int clientNum = ent - g_entities;
	if (clientNum >= 0 && clientNum < MAX_CLIENTS)
		level.mapSelectorVoteByClient[clientNum] = 1;

	P_Menu_Close(ent);
	gi.local_sound(ent, CHAN_AUTO, gi.soundindex("misc/menu3.wav"), 1, ATTN_NONE, 0);
}

static void G_Menu_MapSelector_SelectArenaThree(Entity *ent, MenuHandle *p) {
	int clientNum = ent - g_entities;
	if (clientNum >= 0 && clientNum < MAX_CLIENTS)
		level.mapSelectorVoteByClient[clientNum] = 2;

	P_Menu_Close(ent);
	gi.local_sound(ent, CHAN_AUTO, gi.soundindex("misc/menu3.wav"), 1, ATTN_NONE, 0);
}

static const int mapselectormenu_option1 = 8;
static const int mapselectormenu_option2 = 9;
static const int mapselectormenu_option3 = 10;
static const int mapselectormenu_progressbar = 17;

const PMenu mapselectormenu[] = {
	{ "", MENU_ALIGN_CENTER, nullptr },
	{ "", MENU_ALIGN_CENTER, nullptr },
	{ "", MENU_ALIGN_CENTER, nullptr },
	{ "", MENU_ALIGN_CENTER, nullptr },
	{ "", MENU_ALIGN_CENTER, nullptr },
	{ "", MENU_ALIGN_CENTER, nullptr },
	{ "Vote for the next arena:", MENU_ALIGN_CENTER, nullptr },
	{ "", MENU_ALIGN_CENTER, nullptr },
	{ "", MENU_ALIGN_CENTER, G_Menu_MapSelector_SelectArenaOne },		// select option 1
	{ "", MENU_ALIGN_CENTER, G_Menu_MapSelector_SelectArenaTwo },		// select option 2
	{ "", MENU_ALIGN_CENTER, G_Menu_MapSelector_SelectArenaThree },		// select option 3
	{ "", MENU_ALIGN_CENTER, nullptr },
	{ "", MENU_ALIGN_CENTER, nullptr },
	{ "", MENU_ALIGN_CENTER, nullptr },
	{ "", MENU_ALIGN_CENTER, nullptr },
	{ "", MENU_ALIGN_CENTER, nullptr },
	{ "", MENU_ALIGN_CENTER, nullptr },
	{ "", MENU_ALIGN_LEFT, nullptr }		// progress bar
};

/*
==============================
UpdateMapSelectorProgressBar
==============================
*/
static void UpdateMapSelectorProgressBar(Entity *ent) {
	...
	Q_strlcpy(entries[mapselectormenu_progressbar].text, bar.c_str(), sizeof(entries[mapselectormenu_progressbar].text));
}


void G_Menu_MapSelector_Update(Entity *ent) {
	PMenu *entries = ent->client->menu->entries;

	// Set hostname in menu index 0, limited to 24 characters
	Q_strlcpy(entries[0].text, hostname->string, 25); // 24 chars + null terminator
	
	// Show map filenames or long names

	if (level.mapSelectorVoteCandidates[0]) {
		Q_strlcpy(entries[mapselectormenu_option1].text,
			level.mapSelectorVoteCandidates[0]->longName.empty() ? level.mapSelectorVoteCandidates[0]->filename.c_str() : level.mapSelectorVoteCandidates[0]->longName.c_str(),
			sizeof(entries[mapselectormenu_option1].text));
		entries[mapselectormenu_option1].SelectFunc = G_Menu_MapSelector_SelectArenaOne;
	} else {
		Q_strlcpy(entries[mapselectormenu_option1].text, "", sizeof(entries[mapselectormenu_option1].text));
		entries[mapselectormenu_option1].SelectFunc = nullptr;
	}
	...
	if (level.mapSelectorVoteCandidates[1]) {
		Q_strlcpy(entries[mapselectormenu_option2].text,
			level.mapSelectorVoteCandidates[1]->longName.empty() ? level.mapSelectorVoteCandidates[1]->filename.c_str() : level.mapSelectorVoteCandidates[1]->longName.c_str(),
			sizeof(entries[mapselectormenu_option2].text));
		entries[mapselectormenu_option2].SelectFunc = G_Menu_MapSelector_SelectArenaTwo;
	} else {
		Q_strlcpy(entries[mapselectormenu_option2].text, "", sizeof(entries[mapselectormenu_option2].text));
		entries[mapselectormenu_option2].SelectFunc = nullptr;
	}

	if (level.mapSelectorVoteCandidates[2]) {
		Q_strlcpy(entries[mapselectormenu_option3].text,
			level.mapSelectorVoteCandidates[2]->longName.empty() ? level.mapSelectorVoteCandidates[2]->filename.c_str() : level.mapSelectorVoteCandidates[2]->longName.c_str(),
			sizeof(entries[mapselectormenu_option3].text));
		entries[mapselectormenu_option3].SelectFunc = G_Menu_MapSelector_SelectArenaThree;
	} else {
		Q_strlcpy(entries[mapselectormenu_option3].text, "", sizeof(entries[mapselectormenu_option3].text));
		entries[mapselectormenu_option3].SelectFunc = nullptr;
	}

	UpdateMapSelectorProgressBar(ent);

	P_Menu_Update(ent);
}

void G_Menu_MapSelector_Open(Entity *ent) {
	P_Menu_Open(ent, mapselectormenu, -1, sizeof(mapselectormenu) / sizeof(PMenu), nullptr, G_Menu_MapSelector_Update);
}
