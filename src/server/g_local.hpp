...
namespace std {
	template<>
	struct formatter<SpawnFlags> : formatter<uint32_t> {
		auto format(const SpawnFlags& flags, format_context& ctx) const {
			return formatter<uint32_t>::format(static_cast<uint32_t>(flags.value), ctx);
		}
	};
}
