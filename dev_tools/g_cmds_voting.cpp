// g_cmds_voting.cpp (Game Commands - Voting)
// This file implements the server-side logic for the player voting system.
// It defines the available vote commands (e.g., changing maps, shuffling
// teams), validates vote calls from players, processes the votes, and executes
// the commands if they pass.
//
// Key Responsibilities:
// - Vote Definition: The `vote_cmds` vector defines all possible vote types,
//   their validation logic, and the function to execute upon success.
// - Vote Initiation: `TryStartVote` is the main entry point for a player to
//   call a vote. It performs checks for permissions, cooldowns, and gametype
//   restrictions.
// - Vote Processing: Manages the state of an active vote, tallying "yes" and
//   "no" votes and checking if a majority has been reached.
...
void Vote_Pass_Gametype() {
	GameType gt = GametypeStringToIndex(level.vote.arg.data());
	if (gt == GameType::None)
		return;
...
static bool Vote_Val_Gametype(gentity_t *ent) {
	if (GametypeStringToIndex(gi.argv(2)) == GameType::None) {
...
static void Vote_Pass_Unlagged() {
	...
	gi.cvarForceSet("g_lagCompensation", argi ? "1" : "0");
}
...
void Vote_Pass_Scorelimit() {
	...
	gi.cvarForceSet(G_Fmt("{}limit", GT_ScoreLimitString()).data(), level.vote.arg.data());
}
...
void VoteCommandStore(gentity_t *ent) {
	...
	for (auto ec : active_players()) {
		...
		//gi.localSound(ec, CHAN_AUTO, gi.soundIndex("misc/pc_up.wav"), 1, ATTN_NONE, 0);
		...
		gentity_t *e = ec->client->follow.target ? ec->client->follow.target : ec;
