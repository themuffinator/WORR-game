...
// g_statusbar.hpp (Game Statusbar Builder)
// This header file defines a C++ "builder" class, `statusbar_t`, which provides
// a fluent interface for constructing the complex layout string used by the
// client-side HUD. This system is a modern replacement for manual string
// concatenation, making HUD layout code more readable, maintainable, and less
// prone to errors.
//
// Key Responsibilities:
// - Defines methods for each HUD command (e.g., `xv`, `yv` for positioning,
//   `pic` for drawing images, `ifstat` for conditional rendering).
// - Each method appends the corresponding command and its arguments to an
//   internal `std::stringstream`.
// - Allows for chaining of commands in a single, expressive statement to build
//   the final layout string that is sent to the client via a configstring.
