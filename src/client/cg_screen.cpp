...
cg_screen.cpp (Client Game Screen) This file is responsible for all client-side screen rendering
that is not part of the 3D world view. It manages the Heads-Up Display (HUD), on-screen
notifications, and center-printed messages. Key Responsibilities: - `CG_DrawHUD`: The main entry
point for drawing the HUD, which it does by parsing a layout string received from the server. -
Center Print System: Manages a queue of messages to be displayed in the center of the screen,
handling both instant and "typed-out" text reveals. - Notification System: Manages a list of
messages (like chat or game events) that appear in the top-left corner of the screen and fade
out over time. - `CG_DrawInventory`: Renders the full-screen inventory/item selection menu. -
Handles rendering of numerical stats (health, armor, ammo) using custom graphical number images.
