...
* `WORR.fgd` describes WORR’s expanded entity catalogue, spawn flags, and key/value metadata.
* `Quake2RE-PSX.fgd`, `Ionized.fgd`, and `q1q2.fgd` expose lighting and legacy helpers shipped with the rerelease so that existing documentation continues to work.
* `ericw_tools.fgd` and `kexlights.fgd` provide supplementary compile-time entity definitions for lighting and fog volumes.
