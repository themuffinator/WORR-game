...
Validates map-related configuration filenames, rejecting traversal tokens,
absolute paths, path separators, or illegal characters.
=============
*/
inline bool G_SanitizeMapConfigFilename(std::string_view rawName, std::string& sanitized, std::string& rejectReason)
{
	...
	if (candidate.find(':') != std::string::npos) {
		rejectReason = "contains a drive designator";
		return false;
	}
